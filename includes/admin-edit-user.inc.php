...
if(isset($_POST["edit-user-submit"])) {
    ...
    if (empty($userName)) {
        header("location: ../website/admin.php?error=emptyfields&userName=" . $userName);
        exit();
    } else {

        $sql = "SELECT idUsers, userName, fName, lName, dateOfBirth, address, email, pwd, admin, manager, accountant, active, suspendEnd, passAttempt FROM users WHERE userName='$userName'";
