...
if(isset($_POST["create-user-submit"])){
    ...
    if(empty($firstName) || empty($lastName) || empty($dateOfBirth) || empty($address) || empty($email) || empty($password) || empty($passwordRepeat)){
        header("location: ../website/admin.php?error=emptyfields&firstName=".$firstName."&lastName=".$lastName."&dob=".$dateOfBirth."&address=".$address."&mail=".$email);
        exit();
    }
    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/", $password)) {
        header("location: ../website/admin.php?error=invalidmail_password&firstName=".$firstName."&lastName=".$lastName."&dob=".$dateOfBirth."&address=".$address);
        exit();
    }
    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("location: ../website/admin.php?error=invalidmail&firstName=".$firstName."&lastName=".$lastName."&dob=".$dateOfBirth."&address=".$address);
        exit();
    }
    //Checking if password meets requirements
    elseif (!preg_match('/^(?=.*\d)(?=.*[A-Za-z])[0-9A-Za-z!@#$%.]{8,30}$/', $password) /*|| $password[0]!= "/^[a-zA-Z]$/"*/) {
        header("location: ../website/admin.php?error=invalidpassword&firstName=".$firstName."&lastName=".$lastName."&dob=".$dateOfBirth."&address=".$address."&mail=".$email);
        exit();
    }
    elseif ($password !== $passwordRepeat) {
        header("location: ../website/admin.php?error=passwordcheck&firstName=".$firstName."&lastName=".$lastName."&dob=".$dateOfBirth."&address=".$address."&mail=".$email);
        exit();
    }
    else {
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){
            header("location: ../website/admin.php?error=sqlerror");
            exit();
        }
        else {
            ...
            if ($result = mysqli_query($conn, $sqlNew)) {
                ...
                header("location: ../website/admin.php?error=sqlgetIDfail");
                exit();
            }
            ...
            if (!mysqli_query($conn, $sqlPWD)) {
                header("location: ../website/admin.php?error=sqlerrorInsertPass");
                exit();
            }
            ...
            header("location: ../website/admin.php?signup=success");
            exit();
        }
